 ...
 * @param scale parameter to multiple matrix product by before adding it to ax
 */
void lmatvec(int64_t ilo, int64_t ihi, int64_t jlo, int64_t jhi, int64_t *idx,
    int64_t *jdx, double *vals, double *x, double *ax, double scale)
{
  int64_t i, j;
  int64_t ncols;
  ...
  for (i=ilo; i<=ihi; i++) {
    ...
    for (j=0; j<ncols; j++) {
      ax[i-ilo] += scale*vals[idx[i-ilo]+j]*x[jdx[idx[i-ilo]+j]-jlo];
    }
  }
}

/**
 * Return serial dot product of two vectors
 * @param ilo,iho bounding indices for vectors
 * @param x, y arrays holding vector values
 */
double ldot(int64_t ilo, int64_t ihi, double *x, double *y)
{
  int64_t i;
  int64_t nvals = ihi-ilo+1;
  double ret = 0.0;
  for (i=0; i<nvals; i++) {
    ret += x[i]*y[i];
  }
 ...
 */
void rcg_solve(int s_a, int g_b, int g_ref, int *g_x)
{
  ...
  *g_x = GA_Duplicate(g_b, "dup_x");
  ...
  pbuf = (double*)malloc(maxlen*sizeof(double));
  tbuf = (double*)malloc(maxlen*sizeof(double));
  /* Create buffer that holds modified RHS */
  prhs = (double*)malloc((my_hi-my_lo+1)*sizeof(double));
  ...
  for (i=0; i<nblocks-1; i++) {
    ...
    for (i=my_lo; i<=my_hi; i++) {
      int64_t ncols = m_idx[i+1-my_lo]-m_idx[i-my_lo];
      for (j=0; j<ncols; j++) {
        printf("p[%d] vals[%ld]: %e\n",me,m_idx[i-my_lo]+j,vals[m_idx[i-my_lo]+j]);
      }
    }
  }
  for (i=my_lo; i<=my_hi; i++) {
    int64_t ncols = my_idx[i+1-my_lo]-my_idx[i-my_lo];
    for (j=0; j<ncols; j++) {
      printf("p[%d] my_vals[%ld]: %e\n",me,my_idx[i-my_lo]+j,my_vals[my_idx[i-my_lo]+j]);
    }
  }
#endif
  dbgcnt = 0;
  while (!converged && iter < iteration_max && !recheck_converged) {
    ...
    /* Update value of modified RHS vector */
    for (i=0; i<my_n; i++) {
      prhs[i] = my_rhs[i];
    }
    printf("p[%d] NBLOCKS: %d\n",me,nblocks);
    for (i=0; i<nblocks-1; i++) {
      NGA_Sprs_array_column_distribution64(s_a, offblocks[i], &lo, &hi);
      ...
      NGA_Sprs_array_access_col_block64(s_a, offblocks[i], &m_idx, &m_jdx,
          &ptr);
      vals = (double*)ptr;
      lmatvec(my_lo, my_hi, lo, hi, m_idx, m_jdx, vals, xbuf, prhs, -1.0);
#if 1
      if (dbgcnt == 1) {
        for (j=0; j<my_n; j++) {
          printf("p[%d] xbuf[%d]: %e\n",me,j+(int)lo,xbuf[j]);
    ...
    /* Initialize CG algorithm on diagonal block */
    sub_iter = 0;
    for (i=0; i<my_n; i++) {
      rbuf[i] = prhs[i];
      pbuf[i] = prhs[i];
      if (dbgcnt == 1)
      printf("p[%d] prhs[%d]: %e rhs[%d]: %e diff[%d]: %e\n",me,i+(int)my_lo,prhs[i],i+(int)my_lo,my_rhs[i],
          i+(int)my_lo,prhs[i]-my_rhs[i]);
    }
    dbgcnt++;
    r2 = ldot(my_lo, my_hi, rbuf, rbuf);
    printf("Initial r2: %f\n",r2);
    /* Do a few iterations of CG on local block */
    while (sub_iter < 4) {
      double alpha, beta;
      for (i=0; i<my_n; i++) {
        tbuf[i] = 0.0;
      }
      lmatvec(my_lo, my_hi, my_lo, my_hi, my_idx, my_jdx, my_vals, pbuf, tbuf, 1.0);
#if 0
      for (j=0; j<(my_hi-my_lo+1); j++) {
        printf("p[%d] block: %d xptr[%d]: %e\n",me,me,j,xptr[j]);
      }
#endif
      alpha = ldot(my_lo, my_hi, tbuf, pbuf);
      alpha = r2/alpha;
      for (i=0; i<my_n; i++) {
        xptr[i] += alpha*pbuf[i];
      }
      ...
      r2 = ldot(my_lo, my_hi, rbuf, rbuf);
      beta = r2/beta;
      printf("alpha: %f beta: %f r2: %f\n",alpha,beta,r2);
      for (i=0; i<my_n; i++) {
        pbuf[i] = rbuf[i] + beta*pbuf[i];
      }
      sub_iter++;
#if 1
      {
        ...
        lmatvec(my_lo, my_hi, my_lo, my_hi, my_idx, my_jdx, my_vals, xptr, tbuf, -1.0);
        for (i=0; i<my_n; i++) {
          if (fabs(tbuf[i]) > res) res = tbuf[i];
        }
        printf("RCG Residual: %f\n",res);
      }
#endif
    }
    ...
    for (i = 0; i<nblocks-1; i++) {
      ...
      NGA_Sprs_array_access_col_block64(s_a, offblocks[i], &m_idx, &m_jdx,
          &ptr);
      vals = (double*)ptr;
      lmatvec(my_lo, my_hi, lo, hi, m_idx, m_jdx, vals, xbuf, tbuf, 1.0);
    }
    ...
    for (i=0; i<my_n; i++) {
      /*
      printf("p[%d] tbuf[%d]: %e rhs[%d]: %e xptr[%d]: %e\n",me,i,tbuf[i],i,my_rhs[i],
          i,xptr[i]);
          */
      if (fabs(tbuf[i]) > residual) {
        residual = fabs(tbuf[i]);
      }
    }
    *maxr = residual;
...
void cg_solve(int s_a, int g_b, int *g_x)
{
  ...
  *g_x = GA_Duplicate(g_b, "dup_x");
  ...
  printf("Initial value of residual: %f\n",residual);
  ...
  while (tol > 1.0e-5 && ncnt < iterations) {
    ...
    residual = GA_Ddot(g_r,g_r);
    beta = residual/beta;
    printf("alpha: %f beta: %f residual: %f\n",alpha,beta,residual);
