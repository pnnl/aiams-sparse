 ...
 */
void acg_solve(int s_a, int g_b, int g_ref, int *g_x)
{
  int i, j, nb, icnt, g_cvg, g_maxr, g_dot_tp, g_dot_rr;
  int g_p;
  int nblocks;
  int *list;
  int my_nb;
  ...
  *g_x = GA_Duplicate(g_b, "dup_x");
  ...
  NGA_Allocate(g_cvg);
  GA_Zero(g_cvg);
  /* Create array for holding dot product of t and p */
  g_dot_tp = NGA_Create_handle();
  NGA_Set_data(g_dot_tp,one,&nprocs,C_DBL);
  NGA_Set_chunk(g_dot_tp,&one);
  NGA_Allocate(g_dot_tp);
  GA_Zero(g_dot_tp);
  NGA_Distribution64(g_dot_tp,me,&lo,&hi);
  NGA_Access64(g_dot_tp,&lo,&hi,&dot_tp,&ld64);
  /* Create array for holding dot product of r and r */
  g_dot_rr = NGA_Create_handle();
  NGA_Set_data(g_dot_rr,one,&nprocs,C_DBL);
  NGA_Set_chunk(g_dot_rr,&one);
  NGA_Allocate(g_dot_rr);
  GA_Zero(g_dot_rr);
  NGA_Distribution64(g_dot_rr,me,&lo,&hi);
  NGA_Access64(g_dot_rr,&lo,&hi,&dot_rr,&ld64);
  ...
  residual =  ldot(g_dot_rr, dot_rr, nprocs, my_n, rbuf, rbuf,1);
  printf("p[%d] Initial RESIDUAL: %e\n",me,residual);
  while (iter < iteration_max && !recheck_converged) {
    double resmax;
    double alpha, beta;
    /* perform matrix vector multiplication */
    for (i=0; i<my_n; i++) tbuf[i] = 0.0;
    /* multiply off-diagonal blocks */
    for (i=0; i<nblocks-1; i++) {
      NGA_Sprs_array_column_distribution64(s_a, offblocks[i], &lo, &hi);
      NGA_Get64(g_p,&lo,&hi,xbuf,&ld64);
    ...
    lmatvec(my_lo, my_hi, my_lo, my_hi, my_idx, my_jdx, my_vals, pbuf, tbuf);

    /* estimate value of dot product of t and p */
    if (iter%2 == 0) {
      alpha =  ldot(g_dot_tp, dot_tp, nprocs, my_n, pbuf, tbuf, 0);
    } else {
      alpha =  ldot(g_dot_tp, dot_tp, nprocs, my_n, pbuf, tbuf, 0);
    }
    printf("p[%d] ALPHA: %e\n",me,alpha);
    ...
    beta = residual;
    /* estimate value of dot product of r and r */
    if (iter%2 == 0) {
      residual =  ldot(g_dot_rr, dot_rr, nprocs, my_n, rbuf, rbuf,0);
    } else {
      residual =  ldot(g_dot_rr, dot_rr, nprocs, my_n, rbuf, rbuf,0);
    }
    printf("p[%d] RESIDUAL: %e BETA: %e\n",me,residual,beta);
    beta = residual/beta;
    /*
    for (i=0; i<my_n; i++) {
      printf("p[%d]: %e r[%d]: %e beta: %e\n",i,rbuf[i],i,tbuf[i],beta);
    }
    */
    for (i=0; i<my_n; i++) {
    ...
    *maxr = resmax;
    ...
    if (recheck_converged) printf("p[%d] Recheck converged is TRUE on iteration %d\n",
        me,iter);
    iter++;
  }
  ...
  NGA_Destroy(g_maxr);
  NGA_Destroy(g_err);
  NGA_Destroy(g_dot_tp);
  NGA_Destroy(g_dot_rr);
  NGA_Destroy(g_p);
  free(resbuf);
  free(ebuf);
  free(list);
...
void cg_solve(int s_a, int g_b, int *g_x)
{
  ...
  *g_x = GA_Duplicate(g_b, "dup_x");
  ...
  printf("p[%d] Initial value of residual: %f\n",me,residual);
  ...
  while (tol > 1.0e-5 && ncnt < iterations) {
    ...
    residual = GA_Ddot(g_r,g_r);
    beta = residual/beta;
    printf("p[%d] alpha: %f beta: %f residual: %f\n",me,alpha,beta,residual);
...
int main(int argc, char **argv) {
  ...
  }
  {
    ...
    GA_Print(g_d);
    NGA_Sprs_array_shift_diag(s_a, &rshift);
    NGA_Sprs_array_get_diag(s_a, &g_d);
    GA_Print(g_d);
    rshift = -1.0;
    NGA_Sprs_array_shift_diag(s_a, &rshift);
