...
This repository is a stripped down version of the Global Arrays toolkit,
focusing primarily on the sparse-matrix capabilities being developed in GA. This
repository can only be built using CMake and relies solely on the MPI-based
runtimes developed under comex. Older runtimes in the armci directory are not
available. This repository can be configured with the script

```
rm -rf CMake*

cmake -DGA_RUNTIME:STRING="MPI_PROGRESS_RANK" \
      -DCMAKE_INSTALL_PREFIX:STRING="/qfs/people/d3g293/aiams-sparse/build/install"
\
      ...
      ..
```
which builds GA with the progress ranks runtime. It will also build several test
applications. Most of the tests are focused on solving Laplace's equation on the
unit cube. This problem can easily be scaled up to represent a matrix with a
dimension of over 100 million.
